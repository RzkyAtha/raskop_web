...
/* ─── GALLERY SLIDER (Polaroid Stack) ──────────────── */
function initGallerySlider() {
  const stack    = document.getElementById('gallery-track');
  ...
  photos.forEach((src, i) => {
    ...
    card.innerHTML = `<img src="${src}" alt="Suasana Raskop" loading="lazy" />`;
  ...
  function arrangeCards() {
    cards.forEach(card => {
      ...
      if (offset < -total / 2) offset += total;
      card.dataset.pos = getPos(offset);
    });
    updateCounter();
  }
  ...
  function goTo(idx, direction) {
    ...
    const prevCenter = cards[current];
    current = ((idx % total) + total) % total;

    if (direction === 'next') {
      prevCenter.classList.add('swipe-left');
    } else if (direction === 'prev') {
      prevCenter.classList.add('swipe-right');
    }

    arrangeCards();

    setTimeout(() => {
      prevCenter.classList.remove('swipe-left', 'swipe-right');
      isAnimating = false;
    }, 500);
  ...
  let tx = 0;
  stack.addEventListener('touchstart', e => { tx = e.changedTouches[0].clientX; }, { passive: true });
